 ...
 */
const docParsers = require ('./parsers');
...
var WordExtractor = (function() {
  ...
  let bufferFromDocumentStream = undefined;
  WordExtractor = class WordExtractor {
    static initClass() {
      ...
      bufferFromDocumentStream = function(document, streamName) {
        return documentStream(document, streamName)
          .then(stream => {
            return stream ? streamBuffer(stream) : Promise.resolve(Buffer.concat([]));
          });
      };
    }
    ...
    extractWithStyles(filename) {
      return extractDocument(filename)
        .then(document => {
          return Promise.all([
            bufferFromDocumentStream(document, 'WordDocument'),
            bufferFromDocumentStream(document, '1Table'),
            bufferFromDocumentStream(document, '0Table'),
            bufferFromDocumentStream(document, 'Data')
          ]).then(function (values) {
            return docParsers.processDocStreams(values);
          });
        });
    }
  };
